...
/obj/machinery/party/turntable/attack_hand(mob/living/user as mob)
	...
	var/t = "<body><br><br><br><div align='center'><table border='0'><B>Juke Box Interface</B><br><br><br><br>"
	t += "<A href='?src=\ref[src];on=1'>On</A><br> <A href='?src=\ref[src];off=1'>Off</A>"
	if(disk)
	...
	for(var/datum/turntable_soundtrack/D in turntable_soundtracks)
		t += "<td height='50' width='50'><A href='?src=\ref[src];on=\ref[D]'>[D.name]</A></td>"

	if(disk)
		if(disk.data)
			t += "<td height='50' width='50'><A href='?src=\ref[src];on=\ref[disk.data]'>[disk.data.name]</A></td>"
		else
			t += "<td height='50' width='50'>No Disk</font></td>"
